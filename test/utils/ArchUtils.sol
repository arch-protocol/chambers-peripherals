...
contract ArchUtils is Test {
        ...
        )
    {
        ...
        inputs[2] = Conversor.iToHex(abi.encode(fromToken));
        inputs[3] = Conversor.iToHex(abi.encode(fromTokenAmount));
    ...
    function deployTradeIssuerV3(uint256 chainId) public returns (TradeIssuerV3 tradeIssuer) {
        if (chainId == 137) {
            tradeIssuer = new TradeIssuerV3(POLYGON_WMATIC, POLYGON_CHAMBER_GOD);
            tradeIssuer.addTarget(POLYGON_UNISWAP_ROUTER);
            tradeIssuer.addTarget(POLYGON_ZERO_EX);
            return tradeIssuer;
        }
        return new TradeIssuerV3(ETH_WETH, ETH_CHAMBER_GOD);
    }
    ...
    /**
     * Logs a full redeem and mint quote in console in a JSON-readable format. Used to create new tests.
     */
    ...
    /**
     * Reads a redeemAndMint quote from a JSON file and returns all params in the quote
     * plus an ITradeIssuerV3.ContractCallInstruction[] array
     */
