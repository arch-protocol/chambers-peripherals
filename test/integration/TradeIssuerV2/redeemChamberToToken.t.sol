...
contract TradeIssuerV2IntegrationRedeemChamberToTokenTest is ChamberTestUtils {
    ...
    function testCannotRedeemAddyWithUnderBoughtBaseToken() public {
        ...
        (bytes memory quotes0, uint256 buyAmount0) =
            getQuoteDataForRedeem(componentQuantities[0], tokens["dai"], tokens["usdc"]);
        (bytes memory quotes2, uint256 buyAmount2) =
            getQuoteDataForRedeem(componentQuantities[2], tokens["usdt"], tokens["usdc"]);
        ...
        uint256 minAmountOutputToken =
            ((buyAmount0 + componentQuantities[1] + buyAmount2) * 99 / 100);

        uint256 biggerBaseTokenReturnedThanExpected = minAmountOutputToken * 100;
        ...
        uint256 totalBaseTokenReturned = tradeIssuer.redeemChamberToToken(
            ...
            issuerWizard,
            IERC20(tokens["usdc"]),
            biggerBaseTokenReturnedThanExpected,
            500e18
        );
    ...
    function testSuccessRedeemAddyToUsdc() public {
        ...
        assertEq(IERC20(tokens["usdc"]).balanceOf(alice), totalBaseTokenReturned);
        assertGe(IERC20(tokens["usdc"]).balanceOf(alice), minAmountOutputToken);
    ...
    function testSuccessRedeemAddyToWeth() public {
        ...
        assertEq(IERC20(tokens["weth"]).balanceOf(alice), totalBaseTokenReturned);
        assertGe(IERC20(tokens["weth"]).balanceOf(alice), minAmountOutputToken);
        console.log("Redeemed Tokens:");
        console.log(totalBaseTokenReturned);
        console.log("Remaining DAI:");
        console.log(IERC20(tokens["dai"]).balanceOf(address(tradeIssuer)));
