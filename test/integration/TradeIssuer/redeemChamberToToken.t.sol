...
contract TradeIssuerIntegrationRedeemChamberToTokenTest is ChamberTestUtils {
    ...
    /**
     * [REVERT] Cannot redeem ERC20 that does not exists
     */
    function testFailRedeemChamberToTokenWithUnexistingERC20() public {
        // Mint some amount to alice
        vaultAssets = new address[](1);
        vaults = new address[](1);
        vaultQuantities = new uint256[](1);
    ...
    function testRedeemChamberToTokenVaultAndERC20Assets() public {
        ...
        // Redeem
        uint256 previousAliceBalance = IERC20(address(tokens["weth"])).balanceOf(alice);
        vm.prank(alice);
        uint256 totalOutputTokenReceived = tradeIssuer.redeemChamberToToken(
            ITradeIssuer.IssuanceParams(
                quotes,
                IERC20(tokens["weth"]),
                minAmountOutputToken,
        ...
        assertEq(
            IERC20(address(tokens["weth"])).balanceOf(alice),
            previousAliceBalance + totalOutputTokenReceived
        );
    ...
    function testRedeemChamberToTokenERC20AssetsOnly() public {
        ...
        uint256 previousAliceBalance = IERC20(address(tokens["weth"])).balanceOf(alice);
        vm.prank(alice);
        uint256 totalWETHReceived = tradeIssuer.redeemChamberToToken(
            ITradeIssuer.IssuanceParams(
                quotes,
                IERC20(tokens["weth"]),
                0, // Can be any value since it wont be used at redeemChamber
        ...
        assertEq(
            IERC20(address(tokens["weth"])).balanceOf(alice),
            previousAliceBalance + totalWETHReceived
        );
        assertEq(IERC20(address(baseChamber)).balanceOf(alice), 0);
        assertEq(IERC20(address(baseChamber)).totalSupply(), 0);
        assertEq(IERC20(address(baseConstituents[0])).balanceOf(address(baseChamber)), 0);
     ...
     */
    function testRedeemChamberToTokenWithAConstituentBeingTheOutputToken() public {
        // Mint some amount to alice
        vaultAssets = new address[](1);
        vaults = new address[](1);
        vaultQuantities = new uint256[](1);
        ...
        // Redeem
        uint256 previousAliceBalance = IERC20(address(tokens["dai"])).balanceOf((alice));
        vm.prank(alice);

        uint256 totalDaiReceived = tradeIssuer.redeemChamberToToken(
            ITradeIssuer.IssuanceParams(
                quotes,
                IERC20(tokens["dai"]),
                0, // Can be any value since it wont be used at redeemChamber
        ...
        assertEq(
            IERC20(address(tokens["dai"])).balanceOf(alice), previousAliceBalance + totalDaiReceived
        );
        assertEq(IERC20(address(baseChamber)).balanceOf(alice), 0);
        assertEq(IERC20(address(baseChamber)).totalSupply(), 0);
        assertEq(IERC20(address(baseConstituents[0])).balanceOf(address(baseChamber)), 0);
