...
contract TradeIssuerIntegrationlMintChamberFromNativeTokenTest is ChamberTestUtils {
    ...
    /**
     * [REVERT] Cannot mint without sending ETHER
     */
    function testCannotMintWithoutEther() public {
    ...
    function testCannotMintWithAnUnderboughtAsset() public {
        ...
        assertEq(IERC20(address(baseChamber)).balanceOf(address(tradeIssuer)), 0);
        assertEq(IERC20(address(baseChamber)).balanceOf(alice), 0);
        assertEq(alice.balance, amountWithSlippage);
        assertEq(IERC20(address(components[0])).balanceOf(address(tradeIssuer)), 0);
        assertEq(IERC20(address(components[1])).balanceOf(address(tradeIssuer)), 0);
        assertEq(IERC20(address(vaults[0])).balanceOf(address(tradeIssuer)), 0);
    }
    ...
    function testMintVaultAndERC20Assets() public {
        ...
        vm.prank(alice);
        uint256 totalNativeTokenUsed = tradeIssuer.mintChamberFromNativeToken{
        ...
        assertEq(IERC20(address(baseChamber)).balanceOf(address(tradeIssuer)), 0);
        assertEq(alice.balance, amountWithSlippage - totalNativeTokenUsed);
        assertLe(
    ...
    /**
     * [SUCCESS] Can mint chamber token that only has vault assets.
     */
    ...
    /**
     * [SUCCESS] Can mint chamber token without vault assets.
     */
     ...
     */
    function testMintWithOverboughtVaultConstituent() public {
        vaultAssets = new address[](1);
        vaults = new address[](1);
        vaultQuantities = new uint256[](1);
