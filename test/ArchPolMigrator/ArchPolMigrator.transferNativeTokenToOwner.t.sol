...
contract ArchPolMigratorTransferERC20ToOwner is ArchPolMigratorTest {
     ...
     */
    function testCannotTransferERC20ToOwnerNotAdmin(address randomCaller) public {
        vm.assume(randomCaller != admin);
        vm.prank(randomCaller);
        vm.expectRevert();
        archPolMigrator.transferNativeTokenToOwner();
    }
    ...
    function testTransferERC20ToOwnerAsAdmin(uint128 randomUint) public {
        ...
        deal(address(archPolMigrator), 100 ether);

        uint256 adminPreviousBalance = address(admin).balance;

        vm.prank(admin);
        archPolMigrator.transferNativeTokenToOwner();
        uint256 adminNewBalance = address(admin).balance;

        assertEq(address(archPolMigrator).balance, 0);
        assertEq(adminNewBalance - adminPreviousBalance, 100 ether);
    }
}
